...
>(_options: Partial<UseFormProps<TFieldValues>>): UseFormReturn<TFieldValues> {
  ...
  const { onSuccessUpdateDefaultValues = false } = _options;

  const _fields = {} as Fields<TFieldValues, FieldsKey>;

  const _formState = reactive<TFormState>({
    get isDirty() {
      if (isEmptyObject(_formState.dirtyFields)) return false;
  ...
  const _setFormState = (props: {
    [K in TFormStateKey]?: TFormState[TFormStateKey];
  }) => {
  ...
  const _getDirtyFields = () => {
    ...
    Object.entries(_fields).forEach(([key, val]) => {
      if (val.isDirty) set(dirtyFields, key, true);
    });
  ...
  const _validate = async (
    ...
    if (isEmptyObject(_fields) || isNullOrUndefined(field)) return;
    ...
    if (Object.keys(res || {}).length) _setFormStateError(fieldName, res);
    else _removeFormStateError(fieldName);
  };
  ...
  const reset: UseFormReset<TFieldValues> = (values, keepStateOptions?) => {
    if (!keepStateOptions) keepStateOptions = {} as any;

    if (!values) values = {} as DefaultValues<TFieldValues>;
    ...
    return async (e) => {
      ...
      for (const fieldName in _fields) {
        ...
        if (onSuccessUpdateDefaultValues)
          _options.defaultValues![fieldName] =
            _fields[fieldName].inputValue.value;
      }
  ...
  const createErrorHandler = (fn: SubmitErrorHandler<TFieldValues>) =>
    createErrorHandlerUtil<TFieldValues>(fn);
  const createSubmitHandler = (fn: SubmitHandler<TFieldValues>) =>
    createSubmitHandlerUtil<TFieldValues>(fn);
  ...
  const register: UseFormRegister<TFieldValues> = (
    ...
    const addEventListenerToElement = () => {
      if (isFieldElement(field.el) || _fields[fieldName].isUnregistered) return;
    ...
    const handleValueChange = async (input: InputEvent | any) => {
      ...
      if (validationModeBeforeSubmit.isOnChange) await _onChange(fieldName);
    };
      ...
      ...(vModelBinding === "modelValue" && {
        ...
        onInput: (e: any) => {
          ...
          queueMicrotask(async () => {
            if (!isModelValue) await handleValueChange(e.target?.value);
          });
        },
      }),
  ...
  const unregister: UseFormUnregister<TFieldValues> = (
    ...
    if (!options.keepDirty) unset(_formState.dirtyFields, fieldName as string);

    if (!options.keepError) unset(_formState.errors, fieldName as string);
